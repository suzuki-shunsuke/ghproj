...
complete -o bashdefault -o default -o nospace -F _cli_bash_autocomplete ghproj`)
	return nil
}
...
func (cc *completionCommand) zshCompletionAction(context.Context, *cli.Command) error {
	...
	fmt.Fprintln(cc.stdout, `#compdef ghproj
...
if [ "$funcstack[1]" = "_ghproj" ]; then
  _ghproj "$@"
else
  compdef _ghproj ghproj
fi`)
	return nil
}
